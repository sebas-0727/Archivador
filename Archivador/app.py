...
def copiar_al_historial():
    try:
        ...
        if verificar_historial(tipo, numero, año, modulo, ubicacion):
            return jsonify(success=False, error="Este documento ya está en el historial."), 409  # Cambiado a 409 Conflict

        # Guardar en historial
        guardar_historial(tipo, numero, año, modulo, ubicacion)

        return jsonify(success=True, message="Búsqueda copiada al historial correctamente")
...
def limpiar_historial():
    ...
    try:
        with connection.cursor() as cursor:
            ...
            connection.commit()
            flash('Historial limpiado exitosamente', 'success')  # Mensaje de éxito
    except Exception as e:
        flash(f'Error al limpiar el historial: {str(e)}', 'error')  # Mensaje de error
    finally:
        connection.close()
    return redirect(url_for('historial'))  # Redirigir a la página del historial
...
def cambiar_ubicacion_documento():
    ...
    if 'numero' in request.form and request.form['numero'].strip():
        ...
        return jsonify({
            'success': True,
            'message': f'Document o {tipo} {numero} del año {año} actualizado exitosamente'
        })
